...
flush_nat(){
	...
	if [ "$ip_nat_exist" -ne 0 ]; then
		...
		do
			...
			iptables -t nat -D OUTPUT -p tcp -m set --match-set gfwlist dst -j REDIRECT --to-ports 1280 > /dev/null 2>&1
			iptables -t nat -D OUTPUT -p tcp -m set --match-set black_list dst -j REDIRECT --to-ports 1280 > /dev/null 2>&1
...
apply_nat_rules(){
	...
	iptables -t nat -A OUTPUT -p tcp -m set --match-set gfwlist dst -j REDIRECT --to-ports 1280
	iptables -t nat -A OUTPUT -p tcp -m set --match-set black_list dst -j REDIRECT --to-ports 1280
	#-----------------------FOR 其它服务端口远程连接---------------------
